...
async function updateSchedule(req, res, next) {
  try {
    const schedule = await TextImageSchedule.update(req.user.id, req.params.id, req.body);
    if (!schedule) return res.status(404).json({ success: false, message: 'Schedule not found' });
    await Log.record(req.user.id, 'Text+Image Schedule Updated', { scheduleId: schedule.id });
